/* NEZZA — animated deep-space background */
(() => {
  'use strict';
  ...
  style.textContent = `
    ...
    .space-glow {
      ...
      background:
        radial-gradient(circle at 50% 20%,rgba(70,155,255,.18),transparent 35%),
        linear-gradient(180deg,transparent 60%,rgba(255,255,255,.18) 100%);
    ...
    @media(max-width:700px){
      .universe-nebula{width:90vw;height:90vw;filter:blur(48px);opacity:.34}
      .universe-cloud{width:120vw;left:-10vw}
      .universe-star:nth-child(n+65){display:none}
    }
  `;
  ...
  layer.innerHTML = `
    <div class="universe-space">
      ...
      <div class="shooting-star" style="--top:18%;--duration:6s;--delay:1s"></div>
      <div class="shooting-star" style="--top:42%;--duration:7s;--delay:4s"></div>
      <div class="shooting-star" style="--top:63%;--duration:8s;--delay:7s"></div>
      <div class="space-glow"></div>
    </div>`;
  document.body.prepend(layer);

  const stars = layer.querySelector('.universe-stars');
  const count = Math.min(105, Math.max(65, Math.floor(window.innerWidth / 11)));
  for(let i=0;i<count;i++){
    const s=document.createElement('i');
    s.className='universe-star';
    s.style.setProperty('--x',`${Math.random()*100}%`);
    s.style.setProperty('--y',`${Math.random()*78}%`);
    s.style.setProperty('--size',`${(Math.random()*2+.45).toFixed(2)}px`);
    s.style.setProperty('--alpha',`${(.25+Math.random()*.7).toFixed(2)}`);
    s.style.setProperty('--duration',`${(3+Math.random()*5).toFixed(2)}s`);
    s.style.setProperty('--delay',`${(-Math.random()*6).toFixed(2)}s`);
    s.style.setProperty('--mx',`${((Math.random()-.5)*28).toFixed(1)}px`);
    s.style.setProperty('--my',`${((Math.random()-.5)*34).toFixed(1)}px`);
    stars.appendChild(s);
  }
