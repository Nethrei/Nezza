...
async function startBirthdayMusic(){if(!audio||musicStarted)return musicStarted;try{await audio.play();musicStarted=true;setMusicButton(true);return true}catch(err){return false}}
...
document.addEventListener('pointerdown',async()=>{await startBirthdayMusic()},{passive:true});
